...
export const ReportsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* ==================================================== */}
      {/* VIEW: DEDICATED MOBILE REPORTS (รายงานและสถิติ)       */}
      {/* Exactly matching prototype_mobile_first.html lines 1091-1172 and Screenshot 2026-09-22 200213.png */}
      {/* ==================================================== */}
      <div className="block md:hidden space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400">วิเคราะห์กระแสเงินสด & สิทธิประโยชน์ภาษี</p>
          </div>
          {/* Month Selector Pill */}
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-xl theme-surface border border-slate-200/80 dark:border-white/[0.08] text-[11px] font-semibold text-slate-700 dark:text-slate-300">
            <Calendar className="w-3.5 h-3.5 text-slate-400" />
            <span>{formatThaiDate(new Date().toISOString(), 'long').replace(/^\d+\s+/, '')}</span>
          </div>
        </div>

        {/* 1. Cashflow Runway Card (Neomorphic Depth + Glow) */}
        <div className="p-4 theme-surface rounded-2xl border border-slate-200/80 dark:border-white/[0.08] neo-card space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-500 dark:text-slate-400 font-medium">
              ความอยู่รอดทางการเงิน (Cashflow Runway)
            </span>
            <span
              className={cn(
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black theme-accent-text num-tabular">
              {runway.runwayMonths}
            </span>
            <span className="text-xs font-semibold text-slate-400">
              เดือน (หากไม่มีรายรับเพิ่ม)
            </span>
          </div>
          <div className="w-full bg-slate-200 dark:bg-white/[0.08] h-2 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(100, (runway.runwayMonths / 6) * 100)}%` }}
            />
          </div>
          <p className="text-[10px] text-slate-400">
            คำนวณจากเงินคงเหลือสุทธิเทียบกับค่าใช้จ่ายเฉลี่ยย้อนหลัง
          </p>
        </div>

        {/* 2. Financial Summary 3-Grid Cards */}
        <div className="grid grid-cols-3 gap-2">
          <div className="p-3 theme-surface rounded-xl border border-slate-200/80 dark:border-white/[0.08] neo-card text-center">
            <span className="text-[10px] text-slate-400 block">รายรับรวม</span>
            <span className="text-xs font-black theme-accent-text num-tabular mt-0.5 block">
              {formatCurrency(summary.totalIncome)}
            </span>
          </div>
          <div className="p-3 theme-surface rounded-xl border border-slate-200/80 dark:border-white/[0.08] neo-card text-center">
            <span className="text-[10px] text-slate-400 block">รายจ่ายรวม</span>
            <span className="text-xs font-black theme-expense-text num-tabular mt-0.5 block">
              {formatCurrency(summary.totalExpense)}
            </span>
          </div>
          <div className="p-3 theme-surface rounded-xl border border-slate-200/80 dark:border-white/[0.08] neo-card text-center">
            <span className="text-[10px] text-slate-400 block">เงินออมสุทธิ</span>
            <span className="text-xs font-black text-slate-900 dark:text-white num-tabular mt-0.5 block">
        ...
        <div className="p-4 theme-surface rounded-2xl border border-slate-200/80 dark:border-white/[0.08] neo-card space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 theme-accent-text" />
              <span>สัดส่วนค่าใช้จ่ายตามหมวดหมู่</span>
            </h4>
            <span className="text-[10px] text-slate-400">ยอดรวมเดือนนี้</span>
          </div>
                ...
                return (
                  <div key={cat.categoryId || cat.name} className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-slate-900 dark:text-white font-bold num-tabular">
        ...
        {/* 4. Tax & Thai Chuay Thai Benefits */}
        <div className="p-4 theme-surface rounded-2xl border border-slate-200/80 dark:border-white/[0.08] neo-card space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
              <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />
              <span>สรุปสิทธิประโยชน์และภาษี</span>
            </h4>
            <Link to="/tax" className="text-[11px] theme-accent-text font-bold hover:underline">
              ดูละเอียด
            </Link>
          </div>
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="p-2.5 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-500/20">
              <span className="text-[10px] text-blue-700 dark:text-blue-300 block font-medium">
                ประหยัดจากสิทธิ 60/40
              </span>
              <span className="text-sm font-bold text-blue-700 dark:text-blue-300 num-tabular block mt-0.5">
                {formatCurrency(summary.totalThaiChuayThaiDiscount)}
              </span>
            </div>
            <div className="p-2.5 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl border border-indigo-200 dark:border-indigo-500/20">
              <span className="text-[10px] text-indigo-700 dark:text-indigo-300 block font-medium">
                สิทธิขอคืนภาษี ภ.ง.ด.91
              </span>
              <span className="text-sm font-bold text-indigo-700 dark:text-indigo-300 num-tabular block mt-0.5">
      ...
      {/* ==================================================== */}
      {/* DESKTOP & TABLET VIEW (>= 768px): Original Tabs      */}
      {/* ==================================================== */}
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-blue-200/80 bg-blue-50/50 dark:border-blue-800/80 dark:bg-blue-950/20 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-100 mt-0.5">
                    รัฐช่วยคุณประหยัดเงินไปได้ทั้งหมด{' '}
                    {formatCurrency(summary.totalThaiChuayThaiDiscount)}
                  </p>
                </div>
              </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                            ...
                            {source.type === 'freelance_part_time' && (
              ...
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    <span>
                      สรุปประมาณการภาษีเงินได้บุคคลธรรมดา (ปีภาษี {taxCalculation.taxYear})
                    </span>
                  </CardTitle>
                  <CardDescription>
                    สรุปคำนวณจากทุกแหล่งรายได้ (เงินเดือน, พาร์ทไทม์, ฟรีแลนซ์) และภาษีหัก ณ
                    ที่จ่ายสะสม
                  </CardDescription>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  ...
                  <div className="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800">
                    ...
                    <span className="text-[10px] text-emerald-600 block mt-0.5">
                      {taxCalculation.isEligibleForRefund
                        ? '(ยื่น ภ.ง.ด.90 ขอคืน 100%)'
                        : `(Effective ${taxCalculation.effectiveRate}%)`}
                    </span>
                  </div>
                </div>
                    ...
                    <table className="w-full text-xs text-left">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                        {taxCalculation.brackets.map((b) => (
                          <tr
                            key={b.bracket}
                            className={b.taxableInBracket > 0 ? 'font-medium' : 'text-slate-400'}
                          >
